        ...
        while self._running:
            try:
                ...
                if hasattr(rr2, "isError") and rr2.isError():
                    raise Exception(f"Read error HC202: {rr2}")
...
class ServoControlPanel(QWidget):
    ...
    def _build_ui(self):
        ...
        fpos.addRow("Live Pos (HC202):", self.entry_live)
    ...
    def on_read_now(self):
        ...
        live = self._read_register(202)   # HC202 -> 202
        if trig is not None:
...
class MainWindow(QMainWindow):
    ...
    def _init_ui(self):
        ...
        for i in range(4):
            cam = BAS_CameraWidget(f"Basler Camera {i}", i)
            self.cameras.append(cam)
            r, c = divmod(i, 2)
