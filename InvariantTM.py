...
def invariant_match_template(rgbimage, rgbtemplate, method, matched_thresh, rot_range, rot_interval, scale_range, scale_interval, rm_redundant, minmax, rgbdiff_thresh=float("inf"),flip_template=True):
    """
    ...
    """
    img_gray = cv2.cvtColor(rgbimage, cv2.COLOR_RGB2GRAY).astype(np.uint8)
    template_gray = cv2.cvtColor(rgbtemplate, cv2.COLOR_RGB2GRAY).astype(np.uint8)
    image_maxwh = img_gray.shape
    height, width = template_gray.shape
    all_points = []
    if flip_template==True:
        template_gray=flip_image(template_gray,1)
    if minmax == False:
